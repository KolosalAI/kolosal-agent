...
// Job implementation
Job::Job(const std::string& func_name, const AgentData& parameters)
    : id(UUIDGenerator::generate()), function_name(func_name), parameters(parameters) {}
...
std::string JobManager::submit_job(const std::string& function_name, const AgentData& parameters, 
    ...
    const auto job = std::make_shared<Job>(function_name, parameters);
...
JobStatus JobManager::get__job_status(const std::string& job_id) const {
    std::lock_guard<std::mutex> lock(queue_mutex);
    const auto it = all_jobs.find(job_id);
    return (it != all_jobs.end()) ? it->second->status : JobStatus::FAILED;
}

FunctionResult JobManager::get__job_result(const std::string& job_id) const {
    std::lock_guard<std::mutex> lock(queue_mutex);
...
std::map<std::string, int> JobManager::get__stats() const {
    std::lock_guard<std::mutex> lock(queue_mutex);
    return {
