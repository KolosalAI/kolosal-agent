...
public:
    ...
    void trace(const std::string& message) { kolosal::KolosalLogger::instance().trace(component_name_, message); }
    void debug(const std::string& message) { kolosal::KolosalLogger::instance().debug(component_name_, message); }
    void info(const std::string& message) { kolosal::KolosalLogger::instance().info(component_name_, message); }
    void warn(const std::string& message) { kolosal::KolosalLogger::instance().warn(component_name_, message); }
    void error(const std::string& message) { kolosal::KolosalLogger::instance().error(component_name_, message); }
    void fatal(const std::string& message) { kolosal::KolosalLogger::instance().fatal(component_name_, message); }
...
public:
     ...
     */
    static void setLog_Level(const std::string& level) {
        ...
        kolosal::KolosalLogger::instance().set_Level(log_level);
    }
    
    /**
     * @brief Add file logging with optional rotation
     */
    static bool addFile_Logging(const std::string& filename, 
        ...
        bool success = file_appender->is_Open();
        if (success) {
            kolosal::KolosalLogger::instance().add_Appender(std::move(file_appender));
        }
     ...
     */
    static void configureConsole_Logging(const bool use_colors = true, bool errors_to_stderr = true) {
        // Clear existing appenders and add new console appender
        kolosal::KolosalLogger::instance().clear_Appenders();
        kolosal::KolosalLogger::instance().add_Appender(
     ...
     */
    static void setupProduction_Logging(const std::string& log_file = "", bool quiet_console = false) {
        setLog_Level("information");
        if (!quiet_console) {
            configureConsole_Logging(false, true); // No colors in production
        } else {
            kolosal::KolosalLogger::instance().clear_Appenders();
        }
        
        if (!log_file.empty()) {
            addFile_Logging(log_file, 50, 10); // 50MB files, keep 10 backups
        }
    }
     ...
     */
    static void setupDevelopment_Logging(const std::string& log_file = "") {
        setLog_Level("DEBUG");
        configureConsole_Logging(true, true); // Colors enabled
        
        if (!log_file.empty()) {
            addFile_Logging(log_file, 10, 3); // Smaller files for dev
        }
    }
