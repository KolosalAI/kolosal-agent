...
#ifndef KOLOSAL_AGENT_INCLUDE_WORKFLOW_SEQUENTIAL_SEQUENTIAL_WORKFLOW_HPP_INCLUDED
#define KOLOSAL_AGENT_INCLUDE_WORKFLOW_SEQUENTIAL_SEQUENTIAL_WORKFLOW_HPP_INCLUDED
...
class Logger;
/**
 * @brief Manages y a m l configurable agent functionality
 */
...
public:
    ...
    std::optional<SequentialWorkflow> get__workflow(const std::string& workflow_id) const;
    ...
    std::optional<SequentialWorkflowResult> get__workflow_result(const std::string& workflow_id) const;
    std::map<std::string, std::string> get__workflow_status(const std::string& workflow_id) const;
    std::map<std::string, int> get__executor_metrics() const;
    
    // Workflow building helpers
    SequentialWorkflow create__workflow(const std::string& workflow_id, const std::string& name);
    SequentialWorkflowStep create__step(const std::string& step_id, const std::string& step_name,
                                      const std::string& agent_id, const std::string& function_name);
    
    // Advanced features
    bool validate_workflow(const SequentialWorkflow& workflow) const;
    std::vector<std::string> get__workflow_dependencies(const std::string& workflow_id) const;
...
#endif // KOLOSAL_AGENT_INCLUDE_WORKFLOW_SEQUENTIAL_SEQUENTIAL_WORKFLOW_HPP_INCLUDED
